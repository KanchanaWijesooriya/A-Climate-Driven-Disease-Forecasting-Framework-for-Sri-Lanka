"""
=============================================================================
Leptospirosis Forecasting Pipeline — FINAL (Plateau Reached)
=============================================================================
XGBoost + LightGBM blend, 80/10/10 temporal split, district-level bias correction.
VERY AGGRESSIVE regularization — gap is DATA-LIMITED (not model-limited).
Gap ~1.3 is acceptable for epidemiological forecasting.

FINAL PARAMS (do not tune further):
  XGB: max_depth=2, lr=0.01, subsample=0.4, colsample=0.4, lambda=3, alpha=2
  LGB: max_depth=2, num_leaves=7, lr=0.01, feature_fraction=0.4, bagging=0.4

Usage: python leptospirosis_pipeline.py
=============================================================================
"""
...
print("=" * 70)
print("  Leptospirosis — XGBoost + LightGBM Blend")
print("=" * 70)
...
fig.suptitle(
    f"Leptospirosis — Training Curves\n"
...
ax.set_title('Predictions vs Actual (Train=Pink, Val=Red, Test=Blue)')
...
ax.plot([0, lim], [0, lim], 'k--', lw=2)
ax.set_xlabel('Actual'); ax.set_ylabel('Predicted')
ax.set_title(f'Predictions vs Actual — Test Set\nMAE={m_bld["mae"]:.2f} R²={m_bld["r2"]:.3f}')
ax.grid(True, alpha=0.3)
plt.tight_layout()
...
ax.set_xlabel('Mean Absolute Error'); ax.set_title('Per-District MAE — Test Set')
...
# =============================================================================
# PLOTS — Training Curves WITH Test Curve
# =============================================================================
...
fig.suptitle(
    f"Leptospirosis — Training Curves with Test\n"
...
ax.axhline(xgb_test_mae, color='#1f77b4', linestyle='--', linewidth=2.5, label=f'Test (Final = {xgb_test_mae:.2f})')
ax.axvline(xgb_best, color='gray', linestyle='--', alpha=0.5, label=f'Early Stop ({xgb_best})')
ax.set_xlabel('Boosting Round', fontsize=11, fontweight='bold')
...
if ltr and lva:
    ...
    ax.axhline(lgb_test_mae, color='#1f77b4', linestyle='--', linewidth=2.5, label=f'Test (Final = {lgb_test_mae:.2f})')
    ax.axvline(lgb_best, color='gray', linestyle='--', alpha=0.5, label=f'Early Stop ({lgb_best})')
ax.set_xlabel('Boosting Round', fontsize=11, fontweight='bold')
ax.set_ylabel('MAE', fontsize=11, fontweight='bold')
...
# =============================================================================
# PLOT — Complete Analysis (2x2: curves + predictions)
# =============================================================================

fig, axes = plt.subplots(2, 2, figsize=(14, 10))
fig.suptitle(
    f"Leptospirosis — Complete Training Analysis (Train/Val/Test)\n"
    f"Test MAE={m_bld['mae']:.2f} | XGB Gap={xgb_gap:.3f} LGB Gap={lgb_gap:.3f}",
...
xva = xgb_evals['validation']['mae']
n = min(len(xtr), xgb_best + 100)
ax.plot(np.arange(n), xtr[:n], color='#e377c2', linewidth=2.2, label='Train')
ax.plot(np.arange(n), xva[:n], color='#d62728', linewidth=2.2, label='Validation')
ax.axhline(xgb_test_mae, color='#1f77b4', linestyle='--', linewidth=2.2, label='Test (Final)')
ax.axvline(xgb_best, color='green', linestyle=':', alpha=0.7, linewidth=1.5)
ax.set_xlabel('Boosting Round', fontweight='bold')
ax.set_ylabel('MAE', fontweight='bold')
ax.set_title('XGBoost: Train/Val/Test Curves', fontweight='bold')
...
if ltr and lva:
    n = min(len(ltr), lgb_best + 100)
    ax.plot(np.arange(n), ltr[:n], color='#e377c2', linewidth=2.2, label='Train')
    ax.plot(np.arange(n), lva[:n], color='#d62728', linewidth=2.2, label='Validation')
    ax.axhline(lgb_test_mae, color='#1f77b4', linestyle='--', linewidth=2.2, label='Test (Final)')
    ax.axvline(lgb_best, color='green', linestyle=':', alpha=0.7, linewidth=1.5)
ax.set_xlabel('Boosting Round', fontweight='bold')
ax.set_ylabel('MAE', fontweight='bold')
ax.set_title('LightGBM: Train/Val/Test Curves', fontweight='bold')
ax.legend(fontsize=10)
ax.grid(True, alpha=0.3)

ax = axes[1, 0]
ax.scatter(y_train, xgb_train_pred, alpha=0.4, s=20, color='#e377c2', label='Train')
ax.scatter(y_val, xgb_val_pred, alpha=0.4, s=20, color='#d62728', label='Validation')
...
ax.set_ylabel('Predicted (XGBoost)', fontweight='bold')
ax.set_title('XGBoost Predictions (Train/Val/Test)', fontweight='bold')
ax.legend(fontsize=9)
ax.grid(True, alpha=0.3)

ax = axes[1, 1]
ax.scatter(y_train, lgb_train_pred, alpha=0.4, s=20, color='#e377c2', label='Train')
ax.scatter(y_val, lgb_val_pred, alpha=0.4, s=20, color='#d62728', label='Validation')
ax.scatter(y_test, lgb_test_pred, alpha=0.4, s=20, color='#1f77b4', label='Test')
ax.plot([0, lim], [0, lim], 'k--', lw=2)
ax.set_xlabel('Actual', fontweight='bold')
ax.set_ylabel('Predicted (LightGBM)', fontweight='bold')
ax.set_title('LightGBM Predictions (Train/Val/Test)', fontweight='bold')
ax.legend(fontsize=9)
ax.grid(True, alpha=0.3)
...
print(f"{'Best Boosting Round':<30} {xgb_best:<20} {lgb_best:<20}")
print("-" * 80)
print(f"{'Blend + Correction Test MAE':<30} {m_bld['mae']:.4f}")
print("=" * 80)

print("\n" + "=" * 70)
print(f"  Leptospirosis pipeline complete. Test MAE = {m_bld['mae']:.3f}")
print("=" * 70)
